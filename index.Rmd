...
This slide also uses the .left-column[] and .right-column[] classes. The left column occupies the left 20% of the slide and the right column the right 75% of the slide. Because the left column is so narrow, it looks best when only a couple of words occupy that space
...
This slide also uses the .pull-left[] and .pull-right[] classes. Each column occupies half of the slide space.
...
knitr::kable(head(lemurs[,1:8],3), format = 'html')
```
...
`> Inline code is differentiated primarily by a monospace font face`
